...
export const FileUploader: React.FC<FileUploaderProps> = ({ submissionId, disabled, onChange }) => {
  ...
  useEffect(() => {
    ...
    fetch(`/attachments/${submissionId}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.ok ? r.json() : [])
      .then(setFiles)
      .catch(() => {});
  }, [submissionId, token]);
  ...
  const uploadFiles = async (selectedFiles: FileList | File[]) => {
    ...
    for (const file of Array.from(selectedFiles)) {
      ...
      try {
        const res = await fetch(`/attachments/${submissionId}`, {
          method: 'POST',
  ...
  const handleDelete = async (filename: string) => {
    if (!token) return;
    const res = await fetch(`/attachments/${submissionId}/${encodeURIComponent(filename)}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
